...
基于 Umami 统计数据分析（2026-06-24）：
- 93% 用户 5 秒内跳出，92% 只看一页
- 98.6% 为直接访问，无搜索引擎来源
- 0 回头客
...
| 需求 | 方案 | 优先级 |
|------|------|--------|
| 工具使用次数 / 收藏 | 基于 localStorage 实现，首页展示最常用工具 | 📌 |
| 更新日志板块 | 首页已有 `updates` 区块，通过编辑 `data/updates.json` 维护 | 📌 |
| RSS Feed | 博客更新通知，`pages/blog/` 内容变化时自动更新 | 📌 |
| 首页引导优化 | 新用户首次访问展示工具速览 / 功能介绍浮层 | 📌 |

### P2 — 内容 & 增长（中期）

| 需求 | 方案 | 优先级 |
|------|------|--------|
| 深度工具使用案例 | 配合 Koen 内容战略，每个工具写一篇使用场景文章 | ⏳ |
| 订阅通知 | 邮件 / RSS 订阅入口，用户在首页扫码或提交邮箱 | ⏳ |
| 工具互链 | 文章/案例中自然引用工具，形成内容闭环 | ⏳ |
