...
| 项目目标 | 非商业开源：把常用工具与选型资料沉淀为可长期低成本维护的静态站（非实时服务、无 SLA 合同义务、不做变现） |
...
| SEO：sitemap、meta、canonical 关键页 | Lighthouse/可访问性基线 CI | 工具页键盘快捷键导航 | 「日更 AI 资讯流」、广告与联盟推广 |
...
| F-02 | 在线 JSON 工具 | 非法 JSON 报行/列；宽松模式支持注释/尾逗号；树形视图可浏览 | 通过 | 2026-07 交付 `js/json-workbench.mjs` 与浏览器回归 |
