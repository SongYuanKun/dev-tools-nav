...
The installer uses `set -euo pipefail`, checks `Linger=yes`, every required command, and `docker info >/dev/null`. It fixes the verification directory at `0700` and both files at `0600`. Before rewriting any libexec/unit target, it must disable an existing timer; a first install may accept only an explicit unit-not-found result. It then fails closed if the service is active or activating without stopping it. It backs up all existing targets into mode-`0700` directories on their target filesystems, stages all four new files with final modes, and atomically moves them into place. On staging, move, reload, or final-validation failure, an EXIT trap preserves the original nonzero status, attempts every restore, reloads units, disables the timer again, and verifies disabled/inactive. A failed restore retains the unique old backup and reports both paths; unknown timer safety retains recovery evidence and emits a high-priority error. The commit point occurs only after 4/4 byte/mode and timer-state verification. Backup deletion after commit is best-effort: failure warns and retains recovery paths without rolling back the verified installation. The installer never enables or starts the timer.
