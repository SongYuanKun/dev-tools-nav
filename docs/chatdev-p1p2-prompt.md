...
> **历史归档（不得作为当前实施依据）**
>
> 本文件保留早期 ChatDev 提示词，仅用于追溯。品牌色、页面范围、路线状态和测试现状均可能过时；当前决策以 `README.md`、`docs/roadmap.md` 和已批准设计文档为准。任何虚构推荐语均不得上线。原提示词中的付费产品与商业化入口任务已废弃：本项目是非商业开源项目。
...
```
我有一个个人IP开发者工具导航站（纯静态 HTML/CSS/JS，零构建零依赖），需要完成以下升级（任务3 已废弃）。请为每项输出完整可用的代码文件。
...
【任务3：已废弃，不实施】
原任务为付费知识产品页与商业化入口，已整体废弃。本项目是非商业开源项目：不做付费产品、付费咨询或任何商业化入口，仓库中也不存在 pages/products.html。
