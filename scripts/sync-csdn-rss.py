...
OUT_PATH = os.environ.get("CSDN_OUT_PATH", os.path.join(_REPO_ROOT, "data", "csdn-articles.json"))
MAX_ITEMS = 40
DESC_MAX = 200
...
def main() -> int:
    ...
    if raw is None:
        ...
        try:
            ...
            if os.path.isfile(OUT_PATH):
                print("Keep existing csdn-articles.json (RSS parse failed).")
                return 0
            items = []

        if not items and os.path.isfile(OUT_PATH):
            print("Keep existing csdn-articles.json (no usable RSS items).")
            return 0
